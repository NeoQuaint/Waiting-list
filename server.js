...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        const existingUser = await query('SELECT id FROM waitlist_users WHERE email = $1', [email]);
        ...
        const result = await query(
            ...
            [name, email, phone, gender, age, ipAddress, req.get('User-Agent') || 'unknown']
        );

        await query('UPDATE waitlist_analytics SET total_signups = total_signups + 1, last_updated = NOW()');

        res.json({ success: true, message: 'Successfully added to waitlist!', userId: result.rows[0].id });
    } catch (error) {
...
async function startServer() {
    ...
    app.listen(PORT, () => {
        ...
        if (process.env.NODE_ENV !== 'production') console.log(`   GET /api/signups (dev only)`);
    });
}
